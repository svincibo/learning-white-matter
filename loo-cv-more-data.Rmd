...
tracts <- read_csv("wml_data_mri_tractprofiles_n58_run2_20220609.csv")
...
missing_run2 <- c(35, 53, 58)
brain_anomaly <- c(60)
remove_ids <- c(bad_outlier, missing_tracts, missing_run2, brain_anomaly)
...
predsacc <- predict(mod_acc, newx = x, s = mod_acc$lambda.min)
acc_model_with_draw_covariates <- 
  lm(acc ~ ., bind_cols(
    acc = y[,"acc"], 
    x[, predict(mod_learn, s="lambda.min", type = "nonzero")$which,
      drop=FALSE])) %>%
  predict(newx = x)
...
* I think the right way to do this is not to use _the estimated model_ from drawing to predict accuracy, but to use _the selected regions_ from the drawing model. Then we refit accuracy, using only those regions, but have new slope/intercepts. Otherwise, things are on the wrong scale. 

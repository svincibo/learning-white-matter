---
title: "Quick report, exploration"
author: "DJM"
date: "23/03/2022"
...
## Check the draw duration response

Is it reasonable to use linear slopes for the learning rate?

* Below we plot by across blocks for each picture. 
* Slopes seem reasonable, but need to condition on the image?
...
## Fit a mixed model to estimate the slopes

* Do we need random slopes and intercepts, or just intercepts.
...
fp <- function(x) {
  ...
  data.frame(pval = anova(m1, m2)$`Pr(>Chisq)`[2])
}
pvals <- wr %>%
  group_by(subID) %>%
  group_modify(~ fp(.x))
round(sort(unlist(pvals$pval)), 2)
```

* These are for an anova. Most are pretty large. So our model is:

`drawduration ~ block + (block | stimulus)`


Here we show estimates for the slopes for each subject.
...
* Is it reasonable to summarize by the mean in each subject?
...
* Probably not. These have strong trends, but are pretty smooth
...
* after removing NAs, we lose 9 subjects and
* We also lose the following regions:

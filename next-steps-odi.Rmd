...
library(glmnet)
library(tidyverse)
recog <- read_csv("wml_data_beh_recog_n56_20220601.csv")
wr <- read_csv("wml_data_beh_write_n56_20220601.csv")
tracts <- read_csv("wml_data_mri_tractprofiles_n51_run1_20220425.csv")

# use fractional anisotropy
fa <- tracts %>% dplyr::select(subID, tractname, nodeID, odi)
fa_sub <- fa %>%
  filter(
...
fa_wide_means <- fa_sub %>%
  ...
  summarise(odi = mean(odi, na.rm = TRUE)) %>%
  pivot_wider(names_from = tractname, values_from = odi)
fa_wide <- fa_sub %>%
  filter(nodeID > 20, nodeID < 181) %>%
  dplyr::select(-gr) %>%
  pivot_wider(names_from = c(tractname, nodeID), values_from = odi)
...
fa_sub <- fa_sub %>% filter(subID != 90, subID != 91, subID != 92, subID != 94, subID != 96) # still processing on brainlife
...
fa_wide <- fa_wide %>% filter(subID != 90, subID != 91, subID != 92, subID != 94, subID != 96) # still processing on brainlife
...
fa_wide_means <- fa_wide_means %>% filter(subID != 90, subID != 91, subID != 92, subID != 94, subID != 96) # still processing on brainlife
...
recog_resp <- recog_resp %>% filter(subID != 90, subID != 91, subID != 92, subID != 94, subID != 96) # still processing on brainlife
...
wr <- wr %>% filter(subID != 90, subID != 91, subID != 92, subID != 94, subID != 96) # still processing on brainlife
...
mean_model_lasso <- lapply(all_resp %>% dplyr::select(-subID), fitter_glmnet) %>%
  bind_rows() %>%

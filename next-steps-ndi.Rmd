...
fa <- tracts %>% dplyr::select(subID, tractname, nodeID, ndi)
fa_sub <- fa %>%
  filter(
...
fa_wide_means <- fa_sub %>%
  ...
  summarise(ndi = mean(ndi, na.rm = TRUE)) %>%
  pivot_wider(names_from = tractname, values_from = ndi)
fa_wide <- fa_sub %>%
  filter(nodeID > 20, nodeID < 181) %>%
  dplyr::select(-gr) %>%
  pivot_wider(names_from = c(tractname, nodeID), values_from = ndi)
